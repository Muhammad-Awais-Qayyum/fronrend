...
const CourseContentList: FC<Props> = ({
    ...
    const [visibleSections, setVisisibleSections] = useState<Set<string>>(
        new Set<string>()
    );
    ...
    const toggleSection = (section: string) => {
        ...
        setVisisibleSections(newVisibleSections);
    };

    return (
        <div
            className={`${!isDemo && "sticky top-24 left-0 z-30"
                } mt-[15px] w-full mr-12`}
        >
            {videoSections.map((section: string, index: number) => {
                const isSectionVisible = visibleSections.has(section);

                //filtering video by section
                ...
                return (
                    <div
                        className={`${!isDemo && "border-b w-[95%] border-[#ffffff8e] pb-2"}`}
                        key={section}
                    >
                        ...
                        {isSectionVisible && (
                            <div className="w-full">
                                {sectionVideos.map((item: any, index: number) => {
                                    ...
                                    return (
                                        <div
                                            className={`w-[100%] ${videoIndex === activeVideo ? " bg-slate-200 dark:bg-slate-800" : ""
